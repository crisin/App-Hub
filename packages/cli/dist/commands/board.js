...
boardCommand
    ...
    for (const stage of stagesToShow) {
        ...
        if (items.length === 0 && options.stage) {
            console.log(chalk.dim(`  No items in ${ITEM_STAGE_LABELS[stage] ?? stage}.`));
            continue;
        }
        if (items.length === 0)
            continue;
        console.log(chalk.bold(`\n  ${ITEM_STAGE_LABELS[stage] ?? stage} (${items.length})`));
        for (const item of items) {
            ...
            console.log(`    ${chalk.dim(item.id)} ${item.title} ${pColor(item.priority)}${labels}${assigned}`);
        }
    }
    console.log();
});
boardCommand
    ...
    const item = await withSpinner('Adding item...', () => hubFetch('/api/board', {
        method: 'POST',
        body: JSON.stringify({
            title,
            stage: options.stage,
...
boardCommand
    ...
    if (!ITEM_STAGES.includes(stage)) {
        console.error(chalk.red(`  Invalid stage: ${stage}. Must be one of: ${ITEM_STAGES.join(', ')}`));
        process.exit(1);
    }
    await withSpinner(`Moving to ${stage}...`, () => hubFetch(`/api/board/${id}`, {
        method: 'PATCH',
...
boardCommand
    ...
    .action(async (id) => {
    await withSpinner('Deleting...', () => hubFetch(`/api/board/${id}`, { method: 'DELETE' }));
...
claudeCmd
    ...
    .action(async () => {
    const items = await withSpinner('Loading Claude items...', () => hubFetch('/api/board/claude'));
    if (items.length === 0) {
    ...
    for (const item of items) {
        const labels = item.labels?.length ? chalk.dim(` [${item.labels.join(', ')}]`) : '';
        console.log(`    ${chalk.dim(item.id)} ${item.title} ${chalk.yellow(item.priority)}${labels}`);
        if (item.description) {
...
claudeCmd
    ...
    const item = await withSpinner('Claiming item...', () => hubFetch('/api/board/claude/claim', {
        method: 'POST',
...
claudeCmd
    ...
    .action(async (id) => {
    await withSpinner('Completing item...', () => hubFetch('/api/board/claude/complete', {
        method: 'POST',
