...
export const listCommand = new Command('list')
    ...
    let projects = await withSpinner('Loading projects...', () => hubFetch('/api/projects?sync=true'));
    if (options.status) {
    ...
    for (const p of projects) {
        ...
        console.log(`  ${colorFn(`[${p.status}]`.padEnd(12))} ${chalk.bold(p.name.padEnd(25))} ${chalk.dim(items + ' items')}  ${chalk.dim(p.template)}`);
    }
    console.log();
});
