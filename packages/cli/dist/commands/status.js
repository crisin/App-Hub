...
export const statusCommand = new Command('status')
    ...
    if (options.set) {
        if (!PROJECT_STATUSES.includes(options.set)) {
            console.log(chalk.red(`Invalid status. Valid: ${PROJECT_STATUSES.join(', ')}`));
            return;
        }
        await withSpinner('Updating status...', () => hubFetch(`/api/projects/${slug}`, {
            method: 'PATCH',
        ...
        const project = await withSpinner('Loading project...', () => hubFetch(`/api/projects/${slug}`));
