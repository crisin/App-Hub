...
taskCommand
    ...
    const item = await withSpinner('Adding item...', () => hubFetch(`/api/projects/${options.project}/items`, {
        method: 'POST',
        body: JSON.stringify({
            title,
...
taskCommand
    ...
    const stages = await withSpinner('Loading items...', () => hubFetch(`/api/projects/${options.project}/items`));
    const stageIcons = {
        idea: '💡',
        plan: '📋',
        build: '🔨',
...
taskCommand
    ...
    .requiredOption('-p, --project <slug>', 'Project slug')
    .action(async (itemId, _options) => {
    await withSpinner('Updating item...', () => hubFetch(`/api/items/${itemId}`, {
        method: 'PATCH',
